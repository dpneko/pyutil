...
engine = create_engine("mysql+pymysql://tron:123456@localhost:3306/balance_info_nft_20220411.sql")
...
balance_info_nft_0.to_csv("/Users/xiang/Documents/nft_airdrop/nft_airdrop_20220410.txt", sep='\t', header=False, index=False, float_format='%.0f')


print(f"总量: {balance_info_nft_0['balance'].sum()/1e6:.6f}")
print(f"最少balance: {balance_info_nft_0['balance'].iloc[-1]:.0f}")
print(f"""TRmZhWz9kneZM4wzDnBdYXiqa33e5rrcKN 需要发放 {float(balance_info_nft_0.query("account_address=='TRmZhWz9kneZM4wzDnBdYXiqa33e5rrcKN'")['balance'].iloc[0])/1e6}""")
print(f"""TChvKhEVXKMP19tiQzBchh6Dm7cyZjuPco 需要发放 {float(balance_info_nft_0.query("account_address=='TChvKhEVXKMP19tiQzBchh6Dm7cyZjuPco'")['balance'].iloc[0])/1e6}""")
